...
export class ChannelStore {
  ...
  #channelByAccessSlug(accessSlug) {
    ...
    } while (Object.hasOwn(this.data.channels, generated) || this.#channelByAccessSlug(generated));
    return generated;
  }
  ...
  #validateRequest(channel, request, model, now) {
    const estimate = estimateRequestTokens(request, Boolean(request.allowZeroTokens));
    if (!request.skipModelPolicy && channel.allowed_models.length > 0 && !isModelAllowed(channel.allowed_models, model)) {
...
function normalizeStore(raw) {
  ...
  for (const [id, candidate] of Object.entries(raw.channels)) {
    ...
    if (accessSlug === null) {
      accessSlug = makeUniqueAccessSlug(accessSlugs, channelIds);
    } else {
...
function makeUniqueAccessSlug(usedAccessSlugs, reservedIds) {
  let accessSlug;
  do {
    accessSlug = makeAccessSlug();
  } while (usedAccessSlugs.has(accessSlug) || reservedIds.has(accessSlug));
  return accessSlug;
}
