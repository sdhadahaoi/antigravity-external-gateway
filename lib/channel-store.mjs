...
export class ChannelStore {
  ...
  #channelByVanitySlug(vanitySlug) {
    ...
    let generated;
    do {
      generated = makeVanitySlug();
    } while (
      Object.hasOwn(this.data.channels, generated)
      || this.#channelByAccessSlug(generated)
      || this.#channelByVanitySlug(generated)
    );
    return generated;
  }
...
function normalizeVanitySlug(value) {
  ...
  if (vanitySlug.length < 6) {
    throw new RangeError('vanity_slug must be between 6 and 64 characters');
  }
...
function makeUniqueVanitySlug(usedVanitySlugs, reservedIds, usedAccessSlugs = new Set()) {
  let vanitySlug;
  do {
    vanitySlug = makeVanitySlug();
  } while (
    usedVanitySlugs.has(vanitySlug)
    || reservedIds.has(vanitySlug)
    || usedAccessSlugs.has(vanitySlug)
  );
  return vanitySlug;
}
